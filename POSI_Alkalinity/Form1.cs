...
namespace POSI_Alkalinity
{
    ...
    {
        public double Ca = 78.0;//补充水钙硬度（按碳酸钙计）
        public double Mg = 19.0;//补充水镁硬度（按碳酸钙计）
        public double Na = 100.0;//补充水钠离子
        public double Cl = 100.0;//补充水氯离子
        public double Clmax = 1000.0;//循环水氯离子限值
        public double Cond = 1000.0;//补充水电导率
        public double Alkalinity_Max = 680.0;//循环水碱度控制限值
        public double Alkalinity_Input = 198.0;//补充水碱度
        ...
        public Form1()
        {
            ...
            do//按POSI计算循环水碱度限值
            {
                Alkalinity = Math.Round((COC_Cl * Math.Pow(10, (1 / (Math.Log10(COC_Cl) * Math.Log10(Ca * Mg / (Cl + Na))) + 1))), 2);
            ...
            //    COC_Cl = COC_Cl - Delta_Coc;
            //} while ((Ca_Alkalinity > Ca_Alkalinity_Max) || (Cl_SO4 > Cl_SO4_Max) || (Mg_SiO2 > Mg_SiO2_Max));
            //COC_Cl += Delta_Coc;
